/* JamVOX - 2 przyciski + pedal ekspresji (M-Audio EX-P)
   Arduino Leonardo

   Przycisk A (A2) -> CC # 80 (FootSW1 -> Program Down)
   Przycisk B (A3) -> CC # 81 (FootSW2 -> Program Up)
   Pedal EX-P (A0) -> CC # 11 (Expression)

   PODLACZENIE EX-P (kabel TRS, rozebrany):
     Tip    (wiper potencjometru) -> A0
     Ring   -> GND
     Sleeve -> 5V
   (jesli pedal dziala "odwrotnie", zamien Ring i Sleeve)

   PRZYCISKI: miedzy pinem a GND (INPUT_PULLUP, bez rezystorow).

   CC11: AUTO-ZAKRES 0-127. Skrypt sam uczy sie rzeczywistego min/max
   pedalu (okno 2 s) i mapuje na 0-127.

   OLED 0.96" I2C (SSD1306 lub SH1106): SDA=pin2, SCL=pin3, VCC=5V, GND.
   Biblioteka U8g2 (zainstaluj w Library Manager).

   HELP MENU: przy starcie wyswietla menu pomocy.
   A/B = nawigacja, przytrzymaj 500ms = wejdz.
   5. Button Mode = toggle PULSE/SINGLE (dla Overloud/JamVOX).
*/
...
#define OLED_CHIP_SH1106  2
#define OLED_CHIP         OLED_CHIP_SSD1306

#if OLED_CHIP == OLED_CHIP_SH1106
...
#ifdef DEBUG_ON
  Serial.begin(115200);
  Serial.println("--- JamVOX footswitch/pedal ready (115200) ---");
#endif
}
...
void testMode() {
  ...
  if (Serial.available()) {
    ...
    if (c == 't' || c == 'T') {
      testOn = !testOn;
      Serial.println(testOn ? "TEST MODE ON - sending CC16 every 500ms" : "TEST MODE OFF");
    }
  }
  ...
  if (testOn && millis() - lastTestSend >= 500) {
    ...
    Serial.println("Sent CC16=127 + CC17=127");
    lastTestSend = millis();
  }
#endif
}
...
void readPedal() {
  ...
  if (millis() - windowStart >= RELEARN_MS) {
    if (winMax - winMin >= MIN_SPREAD) {
      ...
      pedalMax = winMax;
#ifdef DEBUG_ON
      Serial.print("Range updated: min=");
...
#ifdef DEBUG_ON
    Serial.print("CC11 val=");
    Serial.print((int)scaled);
    Serial.print("  (raw=");
    Serial.print(raw);
    Serial.println(")");
#endif
...
#ifdef DEBUG_ON
  if (pressed) Serial.print("BTN A -> CC");
  if (pressed) { Serial.print(cc); Serial.println(" pulse"); }
#endif
}
...
#ifdef DEBUG_ON
  if (pressed) Serial.print("BTN B -> CC");
  if (pressed) { Serial.print(cc); Serial.println(" pulse"); }
#endif
}
...
void readButtons() {
  ...
  if (aNow == btnARaw && t - btnADebounceStart >= BTN_DEBOUNCE_MS && aNow != btnAStable) {
    btnAStable = aNow;
  }
  if (bNow != btnBRaw) {
    btnBRaw = bNow;
  ...
  if (bNow == btnBRaw && t - btnBDebounceStart >= BTN_DEBOUNCE_MS && bNow != btnBStable) {
    btnBStable = bNow;
  }
  ...
  if (helpState != HELP_STATE_OFF) {
    if (helpState == HELP_STATE_MENU) {
      ...
      // Widok podmenu (1-3) -> krotkie wcisniecie = powrot do menu
  ...
  // --- Oba przyciski razem (2s) -> toggle FX pedal ---
...
#ifdef DEBUG_ON
      Serial.print("FX pedal toggled -> ");
      Serial.println(fxOff ? "OFF" : "ON");
#endif
    }
    prevStableA = oledBtnA;
    prevStableB = oledBtnB;
  ...
  if (oledBtnA && !btnALongFired && (t - btnAPressStart >= PEDAL_TOGGLE_MS)) {
    ...
    btnBBlink = false;
#ifdef DEBUG_ON
    Serial.print("Button swap toggled -> ");
    Serial.println(btnSwap ? "SWAPPED" : "NORMAL");
#endif
  }
  if (!oledBtnA && prevStableA) {
    sendButtonA(false);
  ...
  if (oledBtnB && !btnBLongFired && (t - btnBPressStart >= PEDAL_TOGGLE_MS)) {
    ...
    btnABlink = false;
#ifdef DEBUG_ON
    Serial.print("Pedal direction toggled -> ");
    Serial.println(pedalInverted ? "INVERTED" : "NORMAL");
#endif
  }
  if (!oledBtnB && prevStableB) {
    sendButtonB(false);
...
void drawHelpItem2() {
  ...
  u8g2.drawBox(0, 0, 128, 10);
  u8g2.setColorIndex(0);
  u8g2.drawStr(24, 8, "CLEAN BUTTON");
  u8g2.setColorIndex(1);
...
void drawHelpItem3() {
  ...
  u8g2.drawBox(0, 0, 128, 10);
  u8g2.setColorIndex(0);
  u8g2.drawStr(25, 8, "BOTH BUTTONS");
  u8g2.setColorIndex(1);
...
void updateOled() {
  if (helpState != HELP_STATE_OFF) {
    ...
    do {
      if (helpState == HELP_STATE_MENU) {
        drawHelpMenu();
      } else if (helpState == HELP_STATE_ITEM1) {
        drawHelpItem1();
      } else if (helpState == HELP_STATE_ITEM2) {
        drawHelpItem2();
      } else if (helpState == HELP_STATE_ITEM3) {
        drawHelpItem3();
      }
    } while (u8g2.nextPage());
    return;
  ...
  do {
    ...
    if (msgOn) {
      ...
      if (showB) {
        u8g2.setColorIndex(0);
        u8g2.drawBox(0, 14, 64, 28);
        u8g2.setColorIndex(1);
        u8g2.drawStr(12, 34, "CLEAN");
      }
      if (showA) {
        u8g2.setColorIndex(0);
        u8g2.drawBox(64, 14, 64, 28);
        u8g2.setColorIndex(1);
        u8g2.drawStr(74, 34, "LEAD");
      }
    }
    ...
    if (fxOff) {
      ...
      u8g2.drawStr((128 - w) / 2, 58, "PED FX OFF");
      u8g2.setColorIndex(1);
    } else {
      u8g2.drawBox(0, 48, 128, 16);
      u8g2.setColorIndex(0);
      int pw = (cc * 128) / 127;
      u8g2.drawBox(pw, 48, 128 - pw, 16);
      u8g2.setColorIndex(1);
      u8g2.setFont(u8g2_font_6x10_tf);
      u8g2.drawBox(54, 50, 20, 12);
      u8g2.setColorIndex(0);
